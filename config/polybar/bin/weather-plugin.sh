...
URL2="api.openweathermap.org/data/2.5/weather?appid=$APIKEY&units=metric&lang=en&q=Memphis,US"
...
function outputCompact {
    OUTPUT="$WIND %{T$WEATHER_FONT_CODE}%{F$ICON_COLOR}$ICON%{F-}%{T-} $ERR_MSG$COLOR_TEXT_BEGIN$DESCRIPTION$COLOR_TEXT_END| $TEMP / $TEMP2%{T2}󰔄%{T-}"
...
if [ $ERROR -eq 0 ]; then
    ...
    TEMP2=`curl -s $URL2 | jq .main.temp`
